...
class Trainer(object):
    def __init__(self, model, loss_function, optimizer, train_loader, validation_loader, batch_size, initial_lr, device, writer, personal_name,
                 log_file, weight_dir,
                 dynamic_lr=False, verbose=True, num_classes=12,resume_loss_state=None,resume_acc_state=None,resume_flag = False):

        self.model = model
        self.loss_function = loss_function
        self.optimizer = optimizer
        self.resume_loss = resume_loss_state
        self.resume_acc = resume_acc_state
    ...
    def val(self, epoch):
        ...
        with torch.no_grad():
            ...
            state = {
                ...
                'avg_loss': running_validation_loss/(print_step + 1),
                'state_dict': self.model.state_dict(),
                'optimizer': self.optimizer.state_dict(),
                'info': self.personal_name
            ...
            if not os.path.exists('./weights/gesture_recog_weights/{}'.format(self.weight_dir)):
                os.makedirs('./weights/gesture_recog_weights/{}'.format(self.weight_dir))
            ...
            if epoch == 0:
                ...
                save_path = './weights/gesture_recog_weights/{}/best_val_acc_checkpoint_{}.pth.tar'.format(
                     self.weight_dir, self.personal_name)
                torch.save(state, save_path)

            else:
                if self.resume_flag:
                    if self.resume_loss and self.resume_acc is None:
                        pass
                    else:
                        self.best_val_loss = self.resume_loss
                        self.best_val_acc = self.resume_acc
                if validation_loss <= self.best_val_loss:
                    self.best_val_loss = validation_loss
                    self.best_val_loss_state = state
                    save_path = './weights/gesture_recog_weights/{}/best_val_loss_checkpoint_{}.pth.tar'.format(
                        self.weight_dir, self.personal_name)
