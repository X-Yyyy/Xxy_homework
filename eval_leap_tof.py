...
from models import C3D, LeNet, LeNet_Leap
import math
import time
...
parser.add_argument('--n_classes', type=int, default=12,
                    help='number of frames per input')
parser.add_argument('--model', type=str,default="MultiRF2_Net") #LeNet MultiRF2_Net
### MultiRF2_Net_leap_motion
# parser.add_argument('--model_path', type=str,default=r"D:\PyCharmresorce\PycharmProjects\learn_pytorch\Xxy_Github_homework\weights\history_model\best_val_acc_checkpoint_MultiRF2_Net_leap_motion_tracking_data_prova.pth.tar")
### LeNet_leap_motion
# parser.add_argument('--model_path', type=str,default=r"D:\PyCharmresorce\PycharmProjects\learn_pytorch\Xxy_Github_homework\weights\history_model\best_val_acc_checkpoint_LeNet_leap_motion_tracking_data_prova.pth.tar")
### LeNet_tof_ir
# parser.add_argument('--model_path', type=str,default=r"D:\PyCharmresorce\PycharmProjects\learn_pytorch\Xxy_Github_homework\weights\history_model\best_val_acc_checkpoint_LeNet_tof_ir_prova.pth.tar")
### MultiRF2_Net_tof_ir
parser.add_argument('--model_path', type=str,default=r"D:\PyCharmresorce\PycharmProjects\learn_pytorch\gestures_recog_MultiRF2\weights\history_model\best_val_acc_checkpoint_MultiRF2_Net_tof_ir_prova.pth.tar")
...
# mode
parser.add_argument('--mode', type=str,default="tof_ir") #### rgb leap_motion_tracking_data tof_ir
parser.add_argument('--gray_scale', action='store_true', default=False)
parser.add_argument('--res_name', type=str)
parser.add_argument('--exp_result_dir', type=str, default="exp_results_all")
...
class GestureTestSet(Dataset):
    def __init__(self, csv_test_path, mode='rgb', normalization_type=-1,
        ...
        with open(self.csv_path, 'r') as csv_in:
            ...
            # depth_ir
            self.list_of_rows_with_first_frame_ir = [row for row in reader if row[4] == 'tof_ir' and row[6] == 'True']
            csv_in.seek(0)
            self.list_of_rows_with_same_mode.append([row for row in reader if row[4] == 'tof_ir'])
            csv_in.seek(0)

            # rgb
            self.list_of_rows_with_first_frame_rgb = [row for row in reader if row[4] == 'rgb' and row[6] == 'True']
            # print("list_of_rows_with_first_frame_rgb:")
            # print(len(self.list_of_rows_with_first_frame_rgb))
            csv_in.seek(0)
            self.list_of_rows_with_same_mode.append([row for row in reader if row[4] == 'rgb'])
            # print("list_of_rows_with_same_mode:")
            # print(len(self.list_of_rows_with_same_mode))
            csv_in.seek(0)
            ...
            # print(len(self.list_of_rows_with_first_frame_tracking_data))
            csv_in.seek(0)
            ...
            #
            # # depth_ir
            file = np.load("mean_std_tof_ir.npz")
            self.mean_ir = file['arr_0']
            self.std_ir = file['arr_1']
            print('mean, std tof_ir loaded.'.format(self.mode))
            ...
            # print(len(self.list_data[2]))
            # print(len(self.list_data[7]))
    ...
    def __getitem__(self, item):
        ...
        if self.mode != 'depth_z' and self.mode != 'tof_ir' and self.mode != 'rgb':
            ...
            if img_data_ir is not None:
                ...
                clip_ir = utilities.create_clip(list_of_img_of_same_record=list_of_img_of_same_record,
                                                   n_frames=self.n_frames,
                                                   mode='tof_ir', resize_dim=self.resize_dim)
    ...
    def __len__(self):
        if self.mode == 'rgb':
            return len(self.list_data[2])
        elif self.mode == 'leap_motion_tracking_data':
            return len(self.list_data[7])
        elif self.mode == 'tof_ir':
            return len(self.list_data[1])
        else:
...
def main():
    ...
    print(args.mode)
    # dataset_name='./csv_test_dataset_rgb.txt'
    # dataset_name='./csv_test_dataset_leap_motion.txt'
    ################
    #ToDo
    ################
    dataset_name='./csv_test_dataset_tof.txt'
    test_set = GestureTestSet(csv_test_path=dataset_name, n_frames=args.n_frames, resize_dim=args.input_size,
                              mode=args.mode, rgb=rgb, model=args.model)
    print("test_set of length : ")
    print(len(test_set))
    test_loader = DataLoader(dataset=test_set, batch_size=args.batch_size)
    print("test_loader of length : ")
